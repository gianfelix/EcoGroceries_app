...
export default function PersonalData() {
  ...
  const handleAction = async (id, action) => {
    ...
    if (action === "Cancel") {
      ...
      try {
        const resCancel = await axios.patch(
          `http://localhost:8000/api/user/cancel/${id}`,
          null,
          {
            headers: {
      ...
      try {
        const resCancel = await axios.patch(
          `http://localhost:8000/api/user/confirm/${id}`,
          null,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const filteredData = apiData.filter((item, index) => {
    ...
    if (!selectedFilter) {
      ...
      const hasStatus = stats.some(
        (status) => status.id_status.toString() === selectedFilter
      );
    ...
    >
          ...
          {isMobileView ? (
            ...
            >
              ...
              <Button
                size={"xs"}
                textColor={"blue.400"}
                onClick={() => checkStat(item)}
              >
                {stats.find((status) => status.id === item.id)?.status}
              </Button>
              {stats.find((status) => status.id === item.id)?.id_status === 4 &&
                stats.find((status) => status.id === item.id)?.showButton && (
                  <Button
                    size={"xs"}
                    textColor={"purple"}
              ...
              {stats.find((status) => status.id === item.id)?.id_status === 1 &&
                stats.find((status) => status.id === item.id)?.showButton && (
                  <>
                    <Button
                      size={"xs"}
                    ...
                    <Button
                      size={"xs"}
                      textColor={"green"}
                      onClick={() => handleAction(item.id, "Pay now")}
                    >
                      Pay now
                    </Button>
                  </>
          ...
          <Flex>
            ...
            <Box ml={"2"}>
              ...
              <Flex>
                ...
                <Text fontSize={"sm"}>
                  {formatCurrencyIDR(
                    item.Transaction_Stocks[0].price /
                      item.Transaction_Stocks[0].qty
                  )}
                </Text>
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalBody>
            {selectedTransaction &&
              selectedTransaction.Transaction_Stocks.map(
                (stock, stockIndex) => (
                  <Box
                    display={"flex"}
                    direction="column"
                  ...
                  >
                    <Image
                      src={`http://localhost:8000/api/${stock.productImg}`}
                      boxSize="100px"
                      objectFit="cover"
                      alt="fruit"
                    />
